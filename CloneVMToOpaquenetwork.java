...
public class CloneVMToOpaquenetwork {

	public boolean cloneVMToOpaquenetwork(ServiceInstance si, String vmName,
			String destHost, String destLSNetwork, String newVMName) {
		...
		try {
			...
			cloneSpec.setLocation(vmRelSpec);
			System.out.println("VM clone started...");
			Folder parent = (Folder) vm.getParent();
			Task task = vm.cloneVM_Task(parent, newVMName, cloneSpec);
			TaskInfo ti = waitFor(task);
			if (ti.getState() == TaskInfoState.error) {
				System.out.println("VM clone failed" + vmName);
				return false;
			}
			System.out.println("VM clone success");
			si.getServerConnection().logout();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			CloneVMToOpaquenetwork vmClone = new CloneVMToOpaquenetwork();
			vmClone.cloneVMToOpaquenetwork(si, vmName, destHost, destLSNetwork,
					newVMName);
		} catch (Exception e) {
			e.printStackTrace();
		}
